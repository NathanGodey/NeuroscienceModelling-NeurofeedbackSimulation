...
def vanilla_feedback(UAF, UAF_thresh):
    return int(UAF < UAF_thresh)


def continuous_feedback(UAF, UAF_thresh):
    return 1 - 2/(1+np.exp(UAF_thresh - UAF))
...
for t in chunk_prog_bar:
    ...
    if t<chunksize:
        ...
        if t == chunksize:
            ...
            feedback = 1-2*continuous_feedback(current_UAF, UAF_thresh)
